buildConvNetModel <- function(use_fc_layer=TRUE, train_data, train_label){
  ...
  conv_1 <- mx.symbol.Convolution(data = data, kernel = c(5, 5), num_filter = 96) # 24x24x20
  tanh_1 <- mx.symbol.Activation(data = conv_1, act_type = "softrelu")
  ...
  conv_2 <- mx.symbol.Convolution(data = dropout_1, kernel = c(5, 5), num_filter = 256) # 6x6x50
  tanh_2 <- mx.symbol.Activation(data = conv_2, act_type = "softrelu")
  ...
  if (use_fc_layer){
    ...
    fc_1 <- mx.symbol.FullyConnected(data = flatten, num_hidden = 1024) # 1x1x500
    tanh_3 <- mx.symbol.Activation(data = fc_1, act_type = "softrelu")
  ...
  devices <- list(mx.gpu(0), mx.gpu(1), mx.gpu(2), mx.gpu(3))
